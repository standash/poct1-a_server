...
def increment_timestamp(timestamp, n_seconds):
    ...
    return "{}-{}-{}T{}-{}".format(chunks[0], chunks[1], xxx[0], "{}:{}:{}".format(time.hour, time.minute, time.second), chunks[3])
...
def generate_ack_message(msg):
    # x_bytez = b""
    # with open ("ack_binary", "rb") as _f:
        # x_bytez = _f.read()
    # print(x_bytez.decode("utf-8"))
    ########################################################
    bytez = b""
    ...
    bytez += "<ACK.control_id V=\"{}\" />".format(msg["control_id"]).encode("utf-8")

    bytez += "</ACK>".encode("utf-8")

    bytez += "</ACK.R01>".encode("utf-8")
    ########################################################
    # print(bytez.decode("utf-8"))
